...
def recieveFile(conn, port, temp, part=False):
    ...
    if part:
        path = f'Server{port}Files\\toBeMerged'
    else:
...
def getFile_route(conn, port , getList = True, splitted = False , splittedFileNames='', part=True):
    if not splitted:
        ...
        if not getList:    
            ...
            file = file.replace('/','',1)
            if part:
                sendFile(conn,f'Server{port}Files\\{file}')
            else:
                try:
...
def splitter(s, port, filename):
    ...
    for i in range(len(ports)):
        names.append(f'Server{port}Files\\{filename}_{i}_{len(ports)}')
        
        filePart = open(names[i], 'wb')
        p.append(filePart)

    f = open(f'Server{port}Files\\{filename}','rb')
    ...
    for i in range(len(ports)):
        p[i].close()
    filename = f'Server{port}Files\\{filename}'
    temp = filename.split('\\')
    temp[1] = f'"{temp[1]}"'
    temp = '\\'.join(temp)
...
def partGetter(port, filename):
    ...
    os.system(f'mkdir Server{port}Files\\toBeMerged')

    part_paths = [f'Server{port}Files\\{filename}']
    ...
    for k in sorted(parts.keys()):
        ...
        if s5.recv(10) == b'Done_5':
            ...
            part_paths.append(f'Server{port}Files\\toBeMerged\\{parts[k][0]}')
        connectorForS5()
    

    comFile = merger(part_paths)
    os.system(f'rmdir /s /q Server{port}Files\\toBeMerged')
    return comFile
...
def merger(fileParts):
    ...
    while r1:
        ...
        f.write(r1)
        check = fileParts[0].split('\\')[-1]
        for i in range(1,len(part),1):
            r2 = part[i].read(1024)
            f.write(r2)
            check = fileParts[i].split('\\')[-1]
        r1 = part[0].read(1024)
    return filename
    pass
